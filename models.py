...
class SS_GDE(nn.Module):
    def __init__(self, args, dict, dict_adj, dict_nnode, dict_label):
        ...
        # self.dist_para = nn.Parameter(torch.randn([args.max_nnode * args.dict_max_nnode, 32]), requires_grad=True)
        ...
        # self.weight_lamda = nn.Parameter(torch.randn(args.classes * args.num_keys * 32, 1), requires_grad=True)

        # dictionary keys
        self.dict = dict
        self.dict = torch.nn.Parameter(self.dict, requires_grad=True)
        self.dict_adj = dict_adj.cuda()
        # self.dict_nnode = dict_nnode.cuda()
