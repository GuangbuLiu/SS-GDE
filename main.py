...
from utils import queue_idx_each_class, convert_data, to_onehot, get_node_max, get_node_unify, subDataset, Calculate_acc


# Training settings
parser = argparse.ArgumentParser()
parser.add_argument('--no-cuda', action='store_true', default=False, help='Disables CUDA training.')
parser.add_argument('--dataset', nargs='?', default='imdb_action_romance', help='mutag,ptc,proteins,collab,imdb_action_romance,imdb_comedy_romance_scifi')
...
parser.add_argument('--dropout', type=float, default=0, help='Dropoute (1 - keep probability).')
...
test_frequency = int(len(labels) * (args.nfold - 1) / args.nfold  / args.batch_size / args.test_fre)
accuracy_test_list = []
for ifold in range(0, args.nfold):
    ...
    for epoch in range(args.epochs):
        ...
        for img_num_i, data in enumerate(dataloader_train):
            ...
            if img_num_i % test_frequency == 0 and img_num_i != 0:
                ...
                print("Test set results:",
                      'loss: {:.4f}'.format(torch.tensor(test_loss_list).mean()),
        ...
        print("Train set results:",
              "epoch:{}".format(epoch),
              'kl_loss:{:.4f}'.format(torch.tensor(kl_loss_list).mean()),
        ...
        print("Test set results:",
              'loss: {:.4f}'.format(torch.tensor(test_loss_list).mean()),
              "acc_test: {:.4f}".format(acc_test.item()), 'best_acc_test: {:.6f}'.format(best_test_acc),)
